"""
Scope: Lahore
Base AQI source: Clarity (via OpenAQ), 2023-11 till live.
...
def fetch_city_base_aqi(city_name, datetime_from, datetime_to):
   
    station = BASE_STATIONS[city_name]
    location_id = station["location_id"]

    sensor_id = get_pm25_sensor_id(location_id)
    if not sensor_id:
        return pd.DataFrame()
    ...
    print(f"  {city_name} station ({station['name']}): {len(df)} rows")

    df = df.sort_values("timestamp").drop_duplicates(subset="timestamp").reset_index(drop=True)
    df["city"] = city_name
    return df[["city", "timestamp", "pm25"]]




def fetch_openmeteo_history(city_name, lat, lng, start_date_str, end_date_str):
    
    start_date = datetime.strptime(start_date_str, "%Y-%m-%d").date()
    end_date = datetime.strptime(end_date_str, "%Y-%m-%d").date()

    
    cutoff_date = end_date - pd.Timedelta(days=5)
    ...
    if start_date <= cutoff_date:
        archive_end = min(end_date, cutoff_date)
        params = base_params.copy()
        params["start_date"] = start_date.strftime("%Y-%m-%d")
        params["end_date"] = archive_end.strftime("%Y-%m-%d")
        ...
        if resp.status_code == 200:
            ...
            print(f"  archive-api error for {city_name}: {resp.text[:150]}")

    
    if end_date > cutoff_date:
        forecast_start = max(start_date, cutoff_date + pd.Timedelta(days=1))
        params = base_params.copy()
        params["start_date"] = forecast_start.strftime("%Y-%m-%d")
        ...
        if resp.status_code == 200:
            ...
            print(f"  forecast-api error for {city_name}: {resp.text[:150]}")
...
def fetch_all_raw(start_dt, end_dt):
    """
    Returns (aqi_df, weather_df), both long format, city column included,
    not merged — the merging + feature engineering happens in feature_pipeline.py
    """
    ...
    for city_name in BASE_STATIONS.keys():
        print(f"\n! {city_name}")
        aqi_df = fetch_city_base_aqi(city_name, datetime_from, datetime_to)
        if not aqi_df.empty:
